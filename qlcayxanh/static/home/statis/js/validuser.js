...
uusermsg = document.getElementById("uname_error_pass");
function checkusername(){
    var error = "";
    var charReg = /^\s*[a-zA-Z0-9,\s]+\s*$/;
    
    if(id_username.value){ // field are filled, let's validate
        var inputVal = id_username.value;
        if (!charReg.test(inputVal)) {
            error = "Error: Username không chấp nhập các chữ số đặt biệt!";
        }
        // //check if either value contains any numbers
        // if(/[0-9]/.test(id_username.value)) 
...
id_username.onchange = checkusername;
...
function checkPassword(){
    ...
    var error = "";
    document.getElementById('uname_error_pass').innerHTML = '';
    document.getElementById('save').disabled = false;
    document.getElementById('usave').disabled = false;
    if(id_password1.value.length==0)
    {
        error = "Error: Hãy nhập mật khẩu!";
        document.getElementById('uname_error_pass').innerHTML = error;
        id_password1.focus();
        document.getElementById('save').disabled = true;
        document.getElementById('usave').disabled = true;
    } else if(id_password1.value.length != 0)
    {
        ...
        if(id_password2.value.length==0)
        {
            ...
            document.getElementById('uname_error_pass').innerHTML = 'Error: Mật khẩu không khớp!';
            id_password2.focus();
            document.getElementById('save').disabled = true;
            document.getElementById('usave').disabled = true;
        }
...
function CheckAllForm(flag) {
    ...
    if (flag!='EDIT_USER'){
        if (document.getElementById('id_password1').value.length == 0){
            document.getElementById("uname_error_pass").innerHTML = 'Error: Hãy nhập mật khẩu!';
        ...
        if (!(lengthPass > 8 || lengthPass < 20)) {
            document.getElementById("uname_error_pass").innerHTML = 'Error: Độ dài mật khẩu phải lớn hơn hoặc bằng 8 ký tự trở lên!';
            document.getElementById("uname_error_pass").focus();
            return false
        }
        ...
        if(document.getElementById('id_password1').value.length != 0 && document.getElementById('id_password1').value == document.getElementById('id_password2').value) {
            if(!checkvalidPassword(document.getElementById('id_password1').value)) {
                document.getElementById("uname_error_pass").innerHTML = 'Error: Mật khẩu phải bao gồm ít nhất 8 ký tự (bao gồm: chữ hoa, chữ thường và số)!';
                document.getElementById("id_password2").focus();
                return false;
            }
        }
